...
export default function CreatePage({
  ...
  return (
    <motion.div key="create" variants={fadeVariants} initial="hidden" animate="visible" exit="exit" className="max-w-4xl mx-auto p-5 md:p-10 flex flex-col gap-6 min-h-full">
      ...
      <div className="flex-1 flex flex-col gap-6">
        ...
        <div className={`${cardClasses} !p-6 flex flex-col md:flex-row gap-5 items-stretch md:items-end bg-gradient-to-br from-white to-violet-50/50 border-violet-100/50`}>
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white border border-violet-100 rounded-xl focus:border-violet-400 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all font-medium text-slate-700 shadow-sm"
            />
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 mt-2">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 px-1">
              ...
              <div className="flex gap-2 w-full md:w-auto">
                ...
                <button onClick={() => onOpenReading(createdText)} className={`${btnSecondary} !py-1.5 !px-4 !rounded-full !text-xs flex-1 md:flex-none !text-violet-600 !border-violet-200 bg-violet-50 hover:bg-violet-100`}>
                  Mở trong Reading
                </button>
              </div>
            </div>
